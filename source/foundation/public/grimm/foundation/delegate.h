...
namespace gm::_detail
{
	...
	struct delegate_vtable
	{
		...
		R(*call)(void* obj, P&&... params);

		constexpr explicit operator bool() const { return move != nullptr; }
	};

	template <typename R> struct delegate_call_helper { template <typename F, typename... P> static R call(F&& f, P&&... p) { return std::invoke(std::forward<F>(f), std::forward<P>(p)...); } };
	template <> struct delegate_call_helper<void> { template <typename F, typename... P> static void call(F&& f, P&&... p) { std::invoke(std::forward<F>(f), std::forward<P>(p)...); } };

	template <typename F> void delegate_move(void* dst, void* src) { new (dst) F(std::move(*static_cast<F*>(src))); }
	template <typename F> void delegate_destruct(void* obj) { static_cast<F*>(obj)->~F(); }
	template <typename F, typename R, typename... P> R delegate_call(void* obj, P&&... params) { return delegate_call_helper<R>::call(*static_cast<F*>(obj), std::forward<P>(params)...); }
...
public:
	...
	delegate& operator=(std::nullptr_t) { reset(); return *this; }
