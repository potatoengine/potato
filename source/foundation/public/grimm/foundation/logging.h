...
#define __gm_LOG(severity, format_string, ...) \
    do { \
        ::gm::logFormattedLine(__FILE__, __LINE__, (severity), "" format_string, __VA_ARGS__); \
    } while (false)
#define __gm_LOG_IF(pred, severity, format_string, ...) \
    do { \
        if (!!(pred)) \
            __gm_LOG((severity), "" format_string, __VA_ARGS__); \
    } while (false)

#if !defined(NDEBUG)
#    define __gm_DEBUG_LOG(severity, format_string, ...) __gm_LOG((severity), "" format_string, __VA_ARGS__)
#    define __gm_DEBUG_LOG_IF(pred, severity, format_string, ...) \
        do { \
            if (!!(pred)) \
                __gm_LOG((severity), "" format_string, __VA_ARGS__); \
        } while (false)
#else
#    define __gm_DEBUG_LOG(severity, format_string, ...) \
        do { \
        } while (false)
#    define __gm_DEBUG_LOG_IF(pred, severity, format_string, ...) \
        do { \
