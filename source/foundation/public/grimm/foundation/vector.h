...
public:
	...
	vector() = default;
	vector(AllocatorT const& allocator) : AllocatorT(allocator) {}
	
	template <typename IteratorT, typename SentinelT> inline vector(IteratorT begin, SentinelT end, AllocatorT const& allocator = AllocatorT{});
	...
	template <typename... ParamsT> auto emplace(const_iterator pos, ParamsT&&... params) -> std::enable_if_t<std::is_constructible<T, ParamsT...>::value, iterator>;
	template <typename... ParamsT> auto emplace_back(ParamsT&&... params) -> std::enable_if_t<std::is_constructible<T, ParamsT...>::value, iterator>;
	...
	template <typename InsertT> iterator insert(const_iterator pos, InsertT&& value) { return emplace(pos, std::forward<InsertT>(value)); }
	...
	template <typename InsertT> iterator push_back(InsertT&& value) { return emplace_back(std::forward<InsertT>(value)); }
...
gm::vector<T, AllocatorT>::vector(IteratorT begin, SentinelT end, AllocatorT const& allocator) : AllocatorT(allocator)
{
	insert(_first, begin, end);
}

template <typename T, typename AllocatorT>
gm::vector<T, AllocatorT>::vector(std::initializer_list<T> initial, AllocatorT const& allocator) : AllocatorT(allocator)
{
	insert(_first, initial.begin(), initial.end());
}

template <typename T, typename AllocatorT>
gm::vector<T, AllocatorT>::vector(vector&& src) : _first(src._first), _last(src._last), _sentinel(src._sentinel), AllocatorT(src)
{
