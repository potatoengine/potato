...
namespace gm
{
	template <typename IteratorT, typename SentinelT = IteratorT> class iterator_range;
	template <typename IteratorT, typename SentinelT> iterator_range<IteratorT, SentinelT> const& make_range(iterator_range<IteratorT, SentinelT> const& range);
	template <typename IteratorT, typename SentinelT> iterator_range<IteratorT, SentinelT> make_range(std::pair<IteratorT, SentinelT> const& range);
	template <typename IteratorT, typename SentinelT> iterator_range<IteratorT, SentinelT> make_range(std::pair<IteratorT, SentinelT>&& range);
	template <typename IteratorT, typename SentinelT> iterator_range<IteratorT, SentinelT> make_range(IteratorT iterator, SentinelT sentinel);
	template <typename ContainerT, typename IteratorT = decltype(std::declval<ContainerT>().begin()), typename SentinelT = decltype(std::declval<ContainerT>().end())> gm::iterator_range<IteratorT, SentinelT> make_range(ContainerT&& container);
}
...
public:
	...
	iterator_range(BeginT&& begin, EndT&& end) : _begin(std::forward<BeginT>(begin)), _end(std::forward<EndT>(end)) {}

	template <typename BeginT, typename EndT>
	/*implicit*/ iterator_range(std::pair<BeginT, EndT> const& range) : _begin(range.first), _end(range.second) {}

	template <typename BeginT, typename EndT>
	/*implicit*/ iterator_range(std::pair<BeginT, EndT>&& range) : _begin(std::move(range.first)), _end(std::move(range.second)) {}
	...
	template <typename = decltype(_end - _begin)> size_type size() const { return _end - _begin; }

	decltype(auto) front() const { return *_begin; }
	template <typename = decltype(_end[-1])> decltype(auto) back() const { return *(_end - 1); }

	void pop_front() { ++_begin; }
	template <typename = decltype(--_end)> void pop_back() { --_end; }
	
	iterator begin() const { return _begin; }
	sentinel end() const { return _end; }
};

template <typename IteratorT, typename SentinelT> struct gm::is_range<gm::iterator_range<IteratorT, SentinelT>> : std::true_type {};
