...
#	include <signal.h>
#	define GM_DEBUG_BREAK() do{ raise(SIGTRAP); }while(false)
#else
...
namespace gm
{
	...
	GM_FRAMEWORK_API GM_NOINLINE error_action fatal_error(char const* file, int line, char const* failedConditionText, char const* messageText);
}
