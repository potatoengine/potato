...
namespace gm
{
	...
	template <typename T, typename... Args>	auto make_box(Args&&...) -> box<T>;
...
{
public:
	using gm::_detail::box_traits<T>::pointer;
	using gm::_detail::box_traits<T>::reference;
	...
	template <typename U> box(box<U>&& src) : _ptr(src.release()) {}
	template <typename U> std::enable_if_t<std::is_assignable_v<T*, U*>, box&> operator=(box<U>&& src)
	{
		if (this != &src)
			reset(src.release());
		return *this;
	...
	template <typename U = T> inline void reset(pointer ptr = nullptr);
	pointer release() { pointer tmp = _ptr; _ptr = nullptr; return tmp; }

	template <typename U> friend class box;

	template <typename U> friend bool operator==(box const& lhs, box<U> const& rhs) { return lhs._ptr == rhs._ptr; }
	template <typename U> friend bool operator!=(box const& lhs, box<U> const& rhs) { return lhs._ptr != rhs._ptr; }
	template <typename U> friend bool operator<=(box const& lhs, box<U> const& rhs) { return lhs._ptr <= rhs._ptr; }
	template <typename U> friend bool operator< (box const& lhs, box<U> const& rhs) { return lhs._ptr <  rhs._ptr; }

	template <typename U> friend bool operator==(box const& lhs, U const* rhs) { return lhs._ptr == rhs; }
	template <typename U> friend bool operator!=(box const& lhs, U const* rhs) { return lhs._ptr != rhs; }
	template <typename U> friend bool operator<=(box const& lhs, U const* rhs) { return lhs._ptr <= rhs; }
	template <typename U> friend bool operator< (box const& lhs, U const* rhs) { return lhs._ptr <  rhs; }

	template <typename U> friend bool operator==(U const* lhs, box const& rhs) { return lhs == rhs._ptr; }
	template <typename U> friend bool operator!=(U const* lhs, box const& rhs) { return lhs != rhs._ptr; }
	template <typename U> friend bool operator<=(U const* lhs, box const& rhs) { return lhs <= rhs._ptr; }
	template <typename U> friend bool operator< (U const* lhs, box const& rhs) { return lhs <  rhs._ptr; }
