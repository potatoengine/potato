...
namespace up {
    ...
    public:
        ...
        /// This is the primary mechanism for finding or mutating Entities.
        ///
        template <typename Callback> void select(World& world, Callback&& callback) requires is_invocable_v<Callback, EntityId, Components&...>;
    ...
    private:
        ...
        void _match();
        template <typename Callback, size_t... Indices> void _executeChunks(World& world, Callback&& callback, std::index_sequence<Indices...>) const;
        template <typename Callback, size_t... Indices> void _execute(World& world, Callback&& callback, std::index_sequence<Indices...>) const;
    ...
    template <typename Callback>
    void Query<Components...>::selectChunks(World& world,
    ...
    template <typename... Components> void Query<Components...>::_match() {
        ...
        for (auto const& match : _matches) {
            for (auto const& chunk : world.chunksOf(match.archetype)) {
                for (unsigned index = 0; index < chunk->header.entities; ++index) {
                    callback(*(static_cast<EntityId*>(static_cast<void*>(chunk->payload)) + index),
