...
up::d3d12::CommandListD3D12::CommandListD3D12(com_ptr<ID3D12DeviceContext> context) : _context(std::move(context)) {}
...
auto up::d3d12::CommandListD3D12::createCommandList(ID3D12Device* device, GpuPipelineState* pipelineState)
    ...
    device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, out_ptr(_commandAllocator));
    ...
    return new_box<CommandListD3D12>(std::move(cl));
}

void up::d3d12::CommandListD3D12::setPipelineState(GpuPipelineState* state) {
    UP_ASSERT(state != nullptr);

    auto pipelineState = static_cast<PipelineStateD3D12*>(state);
...
void up::d3d12::CommandListD3D12::bindRenderTarget(up::uint32 index, GpuResourceView* view) {
    ...
    _rtv[index] = rtv->getView().as<ID3D12RenderTargetView>();
...
void up::d3d12::CommandListD3D12::bindDepthStencil(GpuResourceView* view) {
    ...
    _dsv = dsv->getView().as<ID3D12DepthStencilView>();
    _bindingsDirty = true;
}
...
void up::d3d12::CommandListD3D12::bindIndexBuffer(GpuBuffer* buffer, GpuIndexFormat indexType, up::uint32 offset) {
    ...
    ID3D12Buffer* d3d12Buffer = static_cast<ID3D12Buffer*>(d3dBuffer->buffer().get());

    UINT d3dOffset = static_cast<UINT>(offset);
    _context->IASetIndexBuffer(d3d12Buffer, toNative(indexType), d3dOffset);
}
...
void up::d3d12::CommandListD3D12::bindVertexBuffer(
    ...
    UINT d3dOffset = static_cast<UINT>(offset);
    _context->IASetVertexBuffers(slot, 1, &d3d12Buffer, &d3dStride, &d3dOffset);
}
...
void up::d3d12::CommandListD3D12::bindConstantBuffer(up::uint32 slot, GpuBuffer* buffer, GpuShaderStage stage) {
    ...
    if ((uint32(stage) & uint32(GpuShaderStage::Vertex)) != 0) {
        _context->VSSetConstantBuffers(slot, 1, &d3d12Buffer);
    }
    if ((uint32(stage) & uint32(GpuShaderStage::Pixel)) != 0) {
        _context->PSSetConstantBuffers(slot, 1, &d3d12Buffer);
    }
}
...
void up::d3d12::CommandListD3D12::bindShaderResource(up::uint32 slot, GpuResourceView* view, GpuShaderStage stage) {
    ...
    auto buffer = static_cast<ResourceViewD3D12*>(view);
    ID3D12ShaderResourceView* srv = static_cast<ID3D12ShaderResourceView*>(buffer->getView().get());
...
void up::d3d12::CommandListD3D12::bindSampler(up::uint32 slot, GpuSampler* sampler, GpuShaderStage stage) {
    ...
    auto d3dSampler = static_cast<SamplerD3D12*>(sampler);
    ID3D12SamplerState* nativeSampler = static_cast<ID3D12SamplerState*>(d3dSampler->get().get());
...
void up::d3d12::CommandListD3D12::setPrimitiveTopology(GpuPrimitiveTopology topology) {
    ...
    switch (topology) {
        case GpuPrimitiveTopology::Triangles:
            primitive = D3D12_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
            break;
        case GpuPrimitiveTopology::Lines:
            primitive = D3D12_PRIMITIVE_TOPOLOGY_LINELIST;
            break;
    }
...
void up::d3d12::CommandListD3D12::setViewport(GpuViewportDesc const& viewport) {
    ...
    _context->RSSetViewports(1, &d3d12Viewport);
}
...
void up::d3d12::CommandListD3D12::clearRenderTarget(GpuResourceView* view, glm::vec4 color) {
    ...
    _context->ClearRenderTargetView(
        static_cast<ID3D12RenderTargetView*>(static_cast<ResourceViewD3D12*>(view)->getView().get()),
        c);
}
...
void up::d3d12::CommandListD3D12::clearDepthStencil(GpuResourceView* view) {
    ...
    _context->ClearDepthStencilView(
        static_cast<ID3D12DepthStencilView*>(static_cast<ResourceViewD3D12*>(view)->getView().get()),
        D3D12_CLEAR_DEPTH,
        1.f,
        0);
}
...
void up::d3d12::CommandListD3D12::finish() {
    _context->FinishCommandList(FALSE, out_ptr(_commandList));
}
...
void up::d3d12::CommandListD3D12::flush(bool async) {
...
auto up::d3d12::CommandListD3D12::map(GpuBuffer* buffer, up::uint64 size, up::uint64 offset) -> span<up::byte> {
    ...
    ID3D12Buffer* d3dBuffer = static_cast<BufferD3D12*>(buffer)->buffer().get();
    UP_ASSERT(d3dBuffer != nullptr);

    D3D12_MAPPED_SUBRESOURCE sub = {};
    ...
    //_context->Map(d3dBuffer, 0, writeAll ? D3D12_MAP_WRITE_DISCARD : D3D12_MAP_WRITE_NO_OVERWRITE, 0, &sub);
    _context->Map(d3dBuffer, 0, D3D12_MAP_WRITE_DISCARD, 0, &sub);
...
void up::d3d12::CommandListD3D12::_flushBindings() {
    if (_bindingsDirty) {
        ...
        _context->OMSetRenderTargets(
            ...
            reinterpret_cast<ID3D12RenderTargetView**>(&_rtv),
            _dsv.get());
        for (auto& view : _rtv) {
            view.reset();
        }
