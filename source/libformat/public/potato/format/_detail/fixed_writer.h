...
namespace up {
    ...
    public:
        template <std::size_t Count> constexpr fixed_writer(char (&buffer)[Count]) noexcept : _buffer(buffer), _cursor(buffer), _length(Count) {
            *buffer = char{};
        }
        constexpr fixed_writer(char* buffer, std::size_t length) noexcept : _buffer(buffer), _cursor(buffer), _length(length) { *buffer = char{}; }
