...
static auto errorCodeToResult(std::error_code ec) noexcept -> up::Result {
    if (!ec) {
        return up::Result::Success;
    }

    if (ec.category() == std::system_category()) {
        // FIXME: translate error codes
        return up::Result::System;
    }
    return up::Result::Unknown;
}
...
auto up::NativeBackend::fileStat(zstring_view path, FileStat& outInfo) const -> Result {
    std::error_code ec;
...
auto up::NativeBackend::createDirectories(zstring_view path) -> Result {
    std::error_code ec;
    std::filesystem::create_directories(path.c_str(), ec);
    return errorCodeToResult(ec);
}

auto up::NativeBackend::copyFile(zstring_view from, zstring_view to) -> Result {
    std::error_code ec;
    std::filesystem::copy_file(from.c_str(), to.c_str(), std::filesystem::copy_options::overwrite_existing, ec);
    return errorCodeToResult(ec);
}

auto up::NativeBackend::remove(zstring_view path) -> Result {
    std::error_code ec;
...
auto up::NativeBackend::removeRecursive(zstring_view path) -> Result {
    std::error_code ec;
