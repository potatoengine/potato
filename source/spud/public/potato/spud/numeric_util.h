...
namespace up {
    ...
    template <typename T> constexpr T max(T const& lhs, T const& rhs) { return lhs < rhs ? rhs : lhs; }

    template <typename T> constexpr T min(T const& lhs, T const& rhs) { return lhs < rhs ? lhs : rhs; }
