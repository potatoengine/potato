...
DOCTEST_TEST_SUITE("[potato][spud] vector") {
    ...
    DOCTEST_TEST_CASE("vector push_back") {
        ...
        DOCTEST_CHECK_EQ(vec.size(), 10'000);
        DOCTEST_CHECK_GE(vec.capacity(), 10'000);
        DOCTEST_CHECK_FALSE(vec.empty());

        DOCTEST_CHECK_EQ(vec.begin() + 10'000, vec.end());
        ...
        DOCTEST_CHECK_EQ(vec.back(), 10'000 * 10'000);
    }
    DOCTEST_TEST_CASE("vector insert") {
        vector<int> vec;
        ...
        DOCTEST_CHECK_EQ(vec.back(), 1);
        DOCTEST_CHECK_EQ(vec.front(), 10'000);
    }
