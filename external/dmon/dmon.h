...
//
// Modified 2020 Sean Middleditch:
                                       ...
                                       uint32_t flags, void* user_data)
{
    ...
    dmon__watch_state* watch = &_dmon.watches[_dmon.num_watches];
    watch->id = dmon__make_id((unsigned)(_dmon.num_watches + 1));
    watch->watch_flags = flags;
    watch->watch_cb = watch_cb;
    watch->user_data = user_data;

    dmon__strcpy(watch->rootdir, sizeof(watch->rootdir) - 1, rootdir);
    dmon__winpath(watch->rootdir, sizeof(watch->rootdir), rootdir);
    ...
    if (watch->dir_handle != INVALID_HANDLE_VALUE) {
        ...
        _DMON_LOG_ERRORF("Could not open: %s %d", rootdir, GetLastError());
        LeaveCriticalSection(&_dmon.mutex);
        _InterlockedExchange(&_dmon.modify_watches, 0);
        return dmon__make_id(0);
    }
